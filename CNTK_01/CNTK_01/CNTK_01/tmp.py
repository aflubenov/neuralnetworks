...
def generate_stock_data(minibatch_size):
    ruedas, operaciones = CNTK_01.cargarData("ggal", 250);
    ruedas = CNTK_01.convertirAPorcentaje(ruedas);
    ruedas, operaciones = CNTK_01.generarDatasets(ruedas, operaciones,200);
    ruedas, operaciones = CNTK_01.normalizarDataset(ruedas, operaciones);
    return np.array(ruedas[0:minibatch_size]).astype(np.float32), np.array(operaciones[0:minibatch_size]).astype(np.float32);
